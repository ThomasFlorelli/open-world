...
class TextureSet:
    ...
    def __init__(self, file_path: str, texture_size: int, display_texture_size: int):
        ...
        self.textures = {name: self.load_texture(idx) for name, idx in self.TEXTURE_INDEXES.items()}
          
    def load_texture(self, index):
        x, y = index
        rect = pygame.Rect(x * self.texture_size, y * self.texture_size,  self.texture_size,  self.texture_size)
        return pygame.transform.scale(self.texturesheet.subsurface(rect).copy(), (self.display_texture_size, self.display_texture_size))
...
class DisplayManager:
    ...
    def draw(self, texture_name: str, x: int, y: int):
        ...
        self.renderer.draw_texture(texture, (x * self.texture_set.display_texture_size, y * self.texture_set.display_texture_size))
