...
from render import DisplayManager, TextureSet, Renderer
...
class Game:
    ...
    def draw_viewport(self):
            ...
            viewport_top_left_x, viewport_top_left_y = (px - self.config.VIEWPORT_WIDTH // 2, py - self.config.VIEWPORT_HEIGHT // 2) 
            ...
            for y in range(self.config.VIEWPORT_HEIGHT):
                ...
                for x in range(self.config.VIEWPORT_WIDTH):
                        ...
                        self.display_manager.draw(self.world.get_tile(tile_x, tile_y).type, x, y)
                        
            self.display_manager.draw("player", self.config.VIEWPORT_WIDTH // 2,self.config.VIEWPORT_HEIGHT // 2)
